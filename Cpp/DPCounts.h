...
int climbStairs(int n) {
  if (n <= 0) return 0;
  vector<int> f(3, 1);
  for (int i = 2; i <= n; ++i) f[i % 3] = f[(i - 1) % 3] + f[(i - 2) % 3];
  return f[n % 3];
}
...
// from 1 to n such that there are exactly k inverse pairs. Since the answer may
// be very large, the answer should be modulo 10^9 + 7.
