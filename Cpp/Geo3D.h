...
int bin2ply() {
  ...
  for (int i = 0; i < model.n; ++i) {
    printf("%.8f %.8f %.8f ", model.vertex[i].position[0],
           model.vertex[i].position[1], model.vertex[i].position[2]);
    printf("%.8f %.8f %.8f ", model.vertex[i].normal[0],
           model.vertex[i].normal[1], model.vertex[i].normal[2]);
    printf("%d %d %d\n", model.vertex[i].color[0], model.vertex[i].color[1],
           model.vertex[i].color[2]);
  }
  for (int i = 0; i < model.m; ++i) {
    printf("3 %d %d %d\n", model.face[i].id[0], model.face[i].id[1],
           model.face[i].id[2]);
  }
  printf("\n");
  fclose(stdout);
  ...
  for (int i = 0; i < model.m; ++i) {
    ...
    for (int j = 0; j < 3; ++j) {
      printf("    <%.6f, %.6f, %.6f>",
             model.vertex[model.face[i].id[j]].position[0],
             model.vertex[model.face[i].id[j]].position[1],
             model.vertex[model.face[i].id[j]].position[2]);
      if (j != 2)
        printf(",\n");
      else
        printf("\n");
