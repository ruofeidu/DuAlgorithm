...
int intersects(Segment u, Segment v) {
  return ((intersectsTolerent(u, v)) && (u.a != v.a) && (u.a != v.b) &&
          (u.b != v.a) && (u.b != v.b));
}

// test if point is on segment
int pOnL(Segment l, Point p) {
  return (cross(l.b, p, l.a) == 0) && (((p.x - l.a.x) * (p.x - l.b.x) < 0) ||
...
bool pointInPolygon(int vcount, Polygon ply, Point q) {
  ...
  for (i = 0; i < vcount; ++i) {
    ...
    if ((intersects(l1, l2)) ||
        ((pOnL(l1, ply[(i + 1) % n])) &&
         ((!pOnL(l1, ply[(i + 2) % n])) &&
              (cross(ply[i], ply[(i + 1) % n], l1.a) *
                   cross(ply[(i + 1) % n], ply[(i + 2) % n], l1.a) >
               0) ||
          (pOnL(l1, ply[(i + 2) % n])) &&
              (cross(ply[i], ply[(i + 2) % n], l1.a) *
                   cross(ply[(i + 2) % n], ply[(i + 3) % n], l1.a) >
               0))))
      ++intersections;
  }
