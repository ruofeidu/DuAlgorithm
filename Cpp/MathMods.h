...
int lcm(int x, int y) { return x * y == 0 ? 0 : x * y / gcd(x, y); }
...
vector<int> modeq(int a, int b, int n) {
  ...
  if (b % d > 0) return res;
  int e = (x * (b / d)) % n;
  for (int i = 0; i < d; ++i) res.emplace_back((e + i * (n / d)) % n);
  return res;
}
...
// possible to measure exactly z litres using these two jugs. Time:  O(logn), n
// is the max of (x, y) Space: O(1) B�zout's identity (also called B�zout's
// lemma) https://en.wikipedia.org/wiki/B%C3%A9zout%27s_identity Let a and b be
// integers with greatest common divisor d. Then, there exist integers x and y
// such that ax + by = d. More generally, the integers of the form ax + by are
...
// The remainders of the Euclidean division of an integer n by several integers,
// then one can determine uniquely the remainder of the division of n by the
// product of these integers, under the condition that the divisors are pairwise
// coprime.
