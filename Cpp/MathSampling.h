...
float squareToUniformSpherePdf(const vec3 &v) {
  return (abs(v.x * v.x + v.y * v.y + v.z * v.z - 1.0f) < 1e-6) ? INV_FOURPI
                                                                : 0.0f;
}
...
float squareToUniformHemispherePdf(const vec3 &v) {
  ...
  return (abs(v.x * v.x + v.y * v.y + v.z * v.z - 1.0f) < 1e-6 && theta > 0 &&
          theta < M_PI_2)
             ? INV_TWOPI
             : 0.0f;
}

inline float tent_func(float x) { return (x > 1 || x < -1) ? 0 : 1 - abs(x); }
...
float squareToTentPdf(const vec2 &p) { return tent_func(p.x) * tent_func(p.y); }
...
vec2 hierarchicalSampleWarping(const vec2 &sample, const SquaredImage &a,
                               int level, float xmin, float xmax, float ymin,
                               float ymax, int cy, int cx) {
  std::random_device rand_dev;
  std::mt19937 generator(rand_dev());
  if (level == 10) {
    ...
    if (sample.x < left) {
      if (sample.y < a[level][cy * 2, cx * 2] / left) {
        ...
        return hierarchicalSampleWarping(s, a, level + 1, xmin, center.x, ymin,
                                         center.y, cy * 2, cx * 2);
      } else {
        vec2 s = vec2(dx0(generator), dy1(generator));
        return hierarchicalSampleWarping(s, a, level + 1, xmin, center.x,
                                         center.y, ymax, cy * 2 + 1, cx * 2);
      }
    } else {
      if (sample.y <
          a[level][cy * 2, cx * 2 + 1] / (a[level][cy * 2, cx * 2 + 1] +
                                          a[level][cy * 2 + 1, cx * 2 + 1])) {
        vec2 s = vec2(dx1(generator), dy0(generator));
        return hierarchicalSampleWarping(s, a, level + 1, center.x, xmax, ymin,
                                         center.y, cy * 2, cx * 2 + 1);
      } else {
        vec2 s = vec2(dx1(generator), dy1(generator));
        return hierarchicalSampleWarping(s, a, level + 1, center.x, xmax,
                                         center.y, ymax, cy * 2 + 1,
                                         cx * 2 + 1);
      }
    }
  }
...
float hierarchicalSampleWarping2x2Pdf(const vec2 &p, const SquaredImage &a) {
  ivec2 v = ivec2((int)round(p.x * (1 << a.size())),
                  (int)round(p.y * (1 << a.size())));
  return a[v.x][v.y];
}
}  // namespace MathSampling
