...
struct BoundingBox3f {
  Point3f min;
  Point3f max;
  Point3f getCenter() const { return (max + min) * 0.5f; }
...
struct Mesh {
  ...
  int getTriangleCount() { return numTriangles; }
...
class Octree {
 ...
 protected:
  class Node {
   public:
    ...
    Node(byte _depth) : depth(_depth) { child = vector<Node *>(8); };
  };
 ...
 private:
  void calcSubBoxes(BoundingBox3f &_box, BoundingBoxList &boxes) {
    ...
    boxes[1] = BoundingBox3f(Point3f(pCenter.x, pMin.y, pMin.z),
                             Point3f(pMax.x, pCenter.y, pCenter.z));
    boxes[2] = BoundingBox3f(Point3f(pMin.x, pCenter.y, pMin.z),
                             Point3f(pCenter.x, pMax.y, pCenter.z));
    boxes[3] = BoundingBox3f(Point3f(pMin.x, pMin.y, pCenter.z),
                             Point3f(pCenter.x, pCenter.y, pMax.z));
    boxes[4] = BoundingBox3f(Point3f(pCenter.x, pCenter.y, pMin.z),
                             Point3f(pMax.x, pMax.y, pCenter.z));
    boxes[5] = BoundingBox3f(Point3f(pCenter.x, pMin.y, pCenter.z),
                             Point3f(pMax.x, pCenter.y, pMax.z));
    boxes[6] = BoundingBox3f(Point3f(pMin.x, pCenter.y, pCenter.z),
                             Point3f(pCenter.x, pMax.y, pMax.z));
    boxes[7] = BoundingBox3f(Point3f(pCenter.x, pCenter.y, pCenter.z),
                             Point3f(pMax.x, pMax.y, pMax.z));
  }
 ...
 protected:
  struct BuildStackItem {
   public:
    ...
    bool splitted = false;
    BuildStackItem(BoundingBox3f &_box, TriIndexList &_triangles, Node *_parent,
                   int _child_id = 8)
        : box(_box),
 ...
 public:
  Node *build(BoundingBox3f &_box, TriIndexList &_triangles) {
    ...
    while (!m_stack.empty()) {
      ...
      if (item->triangles.size() < MIN_NUM_TRIANGLES ||
          (item->parent && item->parent->depth > OCTREE_DEPTH_LIMIT)) {
        ++m_total_nodes;
        ++m_leaf_nodes;
        m_total_triangles += (int)item->triangles.size();
        item->node = new LeafNode(item->triangles);
        if (item->parent != nullptr)
          item->parent->child[item->child_id] = item->node;
        m_stack.pop();
        continue;
      }
      calcSubBoxes(item->box, item->boxes);

      // assume that a triangle overlaps with an octree cell if the bounding box
      // of the triangle's vertices overlap with the cell
      ...
      for (int i = 0; i < 8; ++i) {
        m_stack.push(
            BuildStackItem(item->boxes[i], item->sets[i], item->node, i));
      }
    }
    return root;
  }
  ...
  Node *build_DFS(BoundingBox3f &_box, TriIndexList &_triangles) {
    if (_triangles.empty()) return nullptr;
    ...
    for (const auto &triIndex : _triangles) {
      ...
      // uint32_t i0 = m_mesh->getIndices()(0, triIndex), i1 =
      // m_mesh->getIndices()(1, triIndex), i2 = m_mesh->getIndices()(2,
      // triIndex); const Point3f p0 =
      // m_mesh->getVertexPositions().col(triIndex), p1 =
      // m_mesh->getVertexPositions().col(i1), p2 =
      // m_mesh->getVertexPositions().col(i2); Vector3f edge0 = p1 - p2, edge1 =
      // p1 - p0, edge2 = p2 - p0;
    ...
    ++m_interior_nodes;
    Node *node = new Node();
    for (int i = 0; i < 8; ++i) node->child[i] = build_DFS(boxes[i], sets[i]);

    return node;
  }

  void rayIntersect(Ray3f &_ray, Intersection &_its, bool _shadowRay,
                    bool &foundIntersection, uint32_t &resIndex) {
    rayIntersectNode(m_root, m_box, _ray, _its, _shadowRay, foundIntersection,
                     resIndex);
  }

 private:
  void rayIntersectNode(Node *_node, BoundingBox3f _box, Ray3f &_ray,
                        Intersection &_its, bool _shadowRay,
                        bool &foundIntersection, uint32_t &resIndex) {
...
#pragma omp parallel for
    ...
    //		rayIntersectNode(_node->child[i], boxes[i], _ray, _its,
...
#pragma omp parallel for
    ...
    for (int i = 0; i < 8; ++i) {
      nearT[i].second = i;
      intersects[i] = (_node->child[i] != nullptr &&
                       boxes[i].rayIntersect(_ray, nearT[i].first, dummy));
      if (!intersects[i]) nearT[i].first = INFINITY;
    }

    sort(nearT.begin(), nearT.end(),
...
#pragma omp parallel for
    ...
    for (int i = 0; i < 8; ++i) {
      ...
      if (intersects[id] && nearT[i].first < _ray.maxt)
        rayIntersectNode(_node->child[id], boxes[id], _ray, _its, _shadowRay,
                         foundIntersection, resIndex);
    }
  }
 ...
 public:
  Octree(Mesh *_mesh) {
    ...
    for (int i = 0; i < m_mesh->getTriangleCount(); ++i) initialTriSet[i] = i;
    ...
    cout << "Avereage Triangles Per Leaf Node: "
         << (double)m_total_triangles / m_leaf_nodes << endl;
