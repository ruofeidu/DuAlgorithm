...
// 5/21/2010, Ruofei Du in CS 361, SJTU.
...
class PIT {
 ...
 public:
  class Node {
   ...
   public:
    Node(int ss = 0, int kk = 0) : l(nullptr), r(nullptr) {
      s = ss;
      k = kk;
    }
  };
 ...
 public:
  ...
  int Rfs(Node *t, int b, int e, int k) {
    if (b == e) return (t->k);
    int ls = t->l == nullptr ? 0 : t->l->s;
    if (k <= ls)
      return Rfs(t->l, b, (b + e) >> 1, k);
    else
      return Rfs(t->r, ((b + e) >> 1) + 1, e, k - ls);
  }

  int Rfs(int k) {
    if (k >= 1 && k <= root->s)
      return Rfs(root, B, E, k);
    else
      return -1;
  }
  ...
  bool Find(Node *t, int b, int e, int k) {
    ...
    if (k <= (b + e) >> 1)
      return Find(t->l, b, (b + e) >> 1, k);
    else
      return Find(t->r, ((b + e) >> 1) + 1, e, k);
  }
  ...
  for (int i = 0; i < m; ++i) {
    ...
    switch (c) {
      ...
      case 'F':
        ...
        break;
      case 'B':
        fout << pit.Find(k) << endl;
        break;
      case 'A':
        fout << pit.GetRank(k) << endl;
        break;
    }
  }
  return 0;
