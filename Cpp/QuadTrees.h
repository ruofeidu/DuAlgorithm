...
private:
	QuadNode* dfs(const Grid& grid,
		int x, int y, int l) {
		if (l == 1) {
			return new QuadNode(grid[x][y] == 1, true, nullptr, nullptr, nullptr, nullptr);
		}
		int half = l / 2;
		...
		if (topLeftNode->isLeaf && topRightNode->isLeaf &&
			...
			return new QuadNode(topLeftNode->val, true, nullptr, nullptr, nullptr, nullptr);
		}
		return new QuadNode(true, false, topLeftNode, topRightNode, bottomLeftNode, bottomRightNode);
	}
};
