...
// put the depth into the end of e[], record the first time i appears in the
// array, as r[i] d[i] is the depth of e[i] LCA(T, u, V) = E[RMQ(d, r[u],
// r[v])], (r[u] < r[v])
...
class RMQ {
 public:
  ...
  RMQ(vector<int> _nums) {
    ...
    pow2 = vector<int>(M, 1);
    log2 = vector<int>(M, 0);
    for (int i = 1; i < M; ++i) pow2[i] = 2 * pow2[i - 1];
    for (int j = (M >> 1) + 1; j <= M; ++j) log2[j] = log2[j >> 1] + 1;
  }
