...
class Scanning {
  ...
  // isn't one, return 0 instead. Solution: Linear scanning with a left pointer
  // Time: O(n)
  ...
  // the most water. Input: [1,8,6,2,5,4,8,3,7] Output: 49 Time: O(N)
  ...
  // least one number) which has the largest sum and return its sum. Time: O(n)
...
Jump 1 step from index 0 to 1, then 3 steps to the last index.
  */
  ...
  // contain all the characters in T in complexity O(n). Input: S =
  // "ADOBECODEBANC", T = "ABC" Output: "BANC" Solution: Sliding-window Scanning
  // with two pointers, use a hash map to remember the counts in T Time: O(n)
  string minWindow(string s, string t) {
    // Use a vector storing the counts of t's characters
    // +: uneliminated chars in t
    // -: overflow chars in s
    vector<int> d(128, 0);
    for (const auto& c : t) ++d[(int)c];
  ...
  */
  vector<int> smallestRange(vector<vector<int>>& nums) {
    ...
    for (int i = 0; i < (int)nums.size(); ++i) {
      for (int x : nums[i]) a.push_back({x, i});
    }
    sort(a.begin(), a.end());
  ...
  // ranges. Input:  [0,1,2,4,5,7] Output: ["0->2", "4->5", "7"]
  ...
  // you want each basket to only carry one type of fruit each. What is the
  // total amount of fruit you can collect with this procedure?
  ...
  // nums[i]. Solution: maintain l as the left product except itself scan from
  // right to left, maintain with the current product and multiple by it
  vector<int> productExceptSelf(vector<int>& nums) {
    const int n = (int)nums.size();
    vector<int> l(n, 1);
    if (nums.empty()) return l;
  ...
  // determine if it's an additive number. 112358 Time:  O(n^3) Space: O(n)
  bool isAdditiveNumber(string num) {
    ...
    for (size_t i = 1; i < n; ++i) {
      for (size_t j = i + 1; j < n; ++j) {
        ...
        while (cur.size() < n) {
          if (num.substr(p, next.size()) != cur.substr(p, next.size())) break;
          p += next.size();
          a = b;
          b = next;
          next = add(a, b);
        ...
        if (cur.size() == n &&
            num.substr(p, next.size()) == cur.substr(p, next.size()))
          return true;
