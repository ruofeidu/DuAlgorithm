...
namespace SegmentTreeX2 {
	...
	struct line
	{
		double x, y1, y2;	// vertical segments, y1 and y2 are two vertical coordinates
	...
	int main()
	{
		int n, i, num, l, r, c = 0; double area, x1, x2, y1, y2; while (cin >> n, n) {
			...
			n <<= 1;
			sort(Line, Line + n, cmp); sort(ty, ty + n);
			y[0] = ty[0];
			// ty so that y contains unique elements
				for (i = num = 1; i < n; i++) if (ty[i] != ty[i - 1])
					y[num++] = ty[i];
			build(1, 1, num); 
			area = 0;
			for (i = 0; i < n - 1; i++) {
				// segment in tre Correspond(num, Line[i].y1);
				r = Correspond(num, Line[i].y2); if (Line[i].s) // insert to left
					insert(1, l, r); else	// remove right
					Delete(1, l, r);
