...
string twoDigits(const int& num, const unordered_map<int, string>& lookup) {
  ...
  return lookup.find((num / 10) * 10)->second + " " +
         lookup.find(num % 10)->second;
}
...
string join(const vector<string>& strings, const string& delim) {
  ...
  copy(strings.begin(), prev(strings.end()),
       ostream_iterator<string>(imploded, delim.c_str()));
  return imploded.str() + *prev(strings.end());
}

string threeDigits(const int& num, const unordered_map<int, string>& lookup,
                   const string& unit) {
  vector<string> res;
  if (num / 100) {
