...
/*
  ...
  user's news feed. Each item in the news feed must be posted by users who the
  user followed or by the user herself. Tweets must be ordered from most recent
  to least recent.
  ...
  vector<int> getNewsFeed(int userId) {
    ...
    if (!m_msg[userId].empty()) {
      heap.emplace(make_tuple(m_msg[userId].rbegin()->first,
                              m_msg[userId].rbegin(), m_msg[userId].rend()));
    }
    for (const auto& id : m_follow[userId]) {
      if (m_msg[id].size()) {
        heap.emplace(make_tuple(m_msg[id].rbegin()->first, m_msg[id].rbegin(),
                                m_msg[id].rend()));
      }
    }
    vector<int> res;
