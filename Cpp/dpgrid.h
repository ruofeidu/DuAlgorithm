 ...
 public:
  ...
  int minimumTotal(vector<vector<int>>& triangle) {
    ...
    for (int i = n - 2; i >= 0; --i) {
      for (int j = 0; j < i + 1; ++j) {
        f[i % 2][j] =
            triangle[i][j] + min(f[(i + 1) % 2][j], f[(i + 1) % 2][j + 1]);
      }
    }
    return f[0][0];
  }
      ...
      for (int j = 1; j < m; ++j) {
        if (ir[i][j] || ic[i][j])
          ...
          for (int k = 0; k < 4; ++k) {
            ...
            if (irc[y][x] && matrix[i][j] >= matrix[y][x]) irc[i][j] = true;
          }
      }
    }
    for (int i = n - 2; i >= 0; --i) {
      for (int j = m - 2; j >= 0; --j) {
        if (dr[i][j] || dc[i][j])
          ...
          for (int k = 0; k < 4; ++k) {
            ...
            int x = j + dx[k];
            if (y < 0 || x < 0 || y >= n || x >= m) continue;
            if (drc[y][x] && matrix[i][j] >= matrix[y][x]) drc[i][j] = true;
          }
      }
    }
  ...
  */
  ...
  int maximalSquare(vector<vector<char>>& a) {
    ...
    for (int i = 1; i < n; ++i) {
      ...
      for (int j = 0; j < m; ++j) {
        if (a[i][j] == '1') {
          f[i % 2][j] = min(f[i % 2][j - 1],
                            min(f[(i - 1) % 2][j], f[(i - 1) % 2][j - 1])) +
                        1;
          res = max(res, f[i % 2][j]);
        } else {
          f[i % 2][j] = 0;
  ...
  int uniquePathsWithObstacles(vector<vector<int>>& obstacleGrid) {
    ...
    if (n == 0 || m == 0 || obstacleGrid[0][0] || obstacleGrid[m - 1][n - 1])
      return 0;
