...
// Solution: Greedy
// Time: O(n)
// Proof: Sort by end date, if the current course cannot be completed, we have
// to remove one course
// if we remove something longer than the maximum one,
// it is possible we can have a smaller end date which yields the next one to
// the schedule -> contradictory!
...
// 853. Car Fleet [M]
// Given positions and speeds of cars and a target destination, count number of
// fleets
// Time:  O(n log n)
// Space: O(n)
