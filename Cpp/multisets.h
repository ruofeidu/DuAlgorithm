...
/**
 * Multiset is an ordered ascending Red-Black tree. In contrast to set, it
 *allows duplicated elemented. lower_bound(x) returns the first element that >=
 *x
 **/
namespace Multisets {
// 327. Count of Range Sum [H]
// Given an integer array nums, return the number of range sums that lie in
// [lower, upper] inclusive. a:	1, 2, 3, 4, 5 sum:	1, 3, 6, 10, 15 range:
// 7-10 result:	2 + 1 = 3 Note sum may exceed int Time: O(N log N) See
// mergesort.h for another solution.
int countRangeSum(vector<int>& nums, int lower, int upper) {
  ...
  for (size_t i = 0; i < nums.size(); ++i) {
    sum += nums[i];
    res += (int)distance(sums.lower_bound(sum - upper),
...
// most t and the difference between i and j is at most k. O(n log k), or use
// hashing
bool containsNearbyAlmostDuplicate(vector<int>& nums, int k, int t) {
  ...
  for (int i = 0; i < n; ++i) {
    ...
    auto low =
        BST.lower_bound(nums[i] - t);  // the first element that >= nums[i] - t
